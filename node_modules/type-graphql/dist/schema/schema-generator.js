        ...
        this.unionTypesInfo = getMetadataStorage_1.getMetadataStorage().unions.map(unionMetadata => {
            return {
                ...
                type: new graphql_1.GraphQLUnionType({
                    ...
                    types: () => unionMetadata.classTypes.map(objectType => this.objectTypesInfo.find(type => type.target === objectType).type),
                    resolveType: unionMetadata.resolveType
                        ? this.getResolveTypeFunction(unionMetadata.resolveType)
                        : instance => {
                            const instanceTarget = unionMetadata.classTypes.find(ClassType => instance instanceof ClassType);
                            if (!instanceTarget) {
